...
const readout = new MotorReadout({ turnGain: Number(params.get('turngain') ?? 1), turnSign: Number(params.get('turnsign') ?? 1), source: params.get('readout') ?? 'dng02', gate: Number(params.get('gate') ?? 1), recenterTau: Number(params.get('recenter') ?? 0) });
