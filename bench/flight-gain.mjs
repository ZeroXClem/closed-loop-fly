...
const GAINS = arg('gains', '1,2,3').split(',').map(Number), SECONDS = Number(arg('seconds', 20)), ADAPT = arg('adapt', ''), TAG = arg('tag', ''), WARM = Number(arg('warm', 3)), NODRUM = process.argv.includes('--no-drum');
...
const report = { gains: GAINS, seconds: SECONDS, warm: WARM, adapt: ADAPT, readout: 'dna02, gate on, recenter 10 s', runs: [] };
for (const flight of GAINS) {
  let page, boot, drum = null;
  if (!NODRUM) {
  // drum assay
  page = await browser.newPage();
  boot = await openLoop(page, vite.url, { ...common, flight });
  ...
  page = await browser.newPage();
  boot = await openLoop(page, vite.url, { ...common, flight });
  if (boot.backend !== 'gpu') throw Error(`LIF fell back to ${boot.backend}`);
  const c = await cruise(page, { warm: WARM, seconds: SECONDS, baseAmp: 0.7 });
  log(`   cruise: ${fmt(c)}`);
  await page.close();
  report.runs.push({ flight, warm: WARM, drum, cruise: c });
...
for (const r of report.runs) console.log(`| ${r.flight} (warm ${r.warm} s) | ${r.drum ? `${r.drum.cw.aHsL} / ${r.drum.cw.aHsR}` : '—'} | ${r.drum ? `${r.drum.cw.bHsL} / ${r.drum.cw.bHsR}` : '—'} | ${r.drum ? `${r.drum.dna02Dsi.L} / ${r.drum.dna02Dsi.R}` : '—'} | ${r.drum ? `${r.drum.dng02Dsi.L} / ${r.drum.dng02Dsi.R}` : '—'} | ${r.cruise.driftDeg}° | ${r.cruise.collisions} | ${r.cruise.wobbleRadS} rad/s |`);
