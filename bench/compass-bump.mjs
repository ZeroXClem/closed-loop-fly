...
const TONICS = arg('tonics', '0.2,0.28,0.34').split(',').map(Number), PULSE = Number(arg('pulse', 1)), PEN_I = Number(arg('pen', 0.5)), ADAPT = arg('adapt', '') ? arg('adapt', '').split(',').map(Number) : null, OUT = arg('out', 'bench/out/compass-bump.json'), NOPEN = process.argv.includes('--no-pen');
...
const wedge = epg.filter((_, k) => Math.abs(Math.atan2(Math.sin(ang[k]), Math.cos(ang[k]))) < Math.PI / 8); // ±22.5° around 0
console.log(`EPG ${epg.length} (wedge ${wedge.length} cells around 0°), PEN ${penL.length} L / ${penR.length} R, Delta7 ${d7.length}${ADAPT ? `, adaptation ${ADAPT}` : ''}`);
...
const report = { tonics: TONICS, pulse: PULSE, pen: PEN_I, adapt: ADAPT, wedge: wedge.length, A: [], B: [] };
...
for (const tonic of TONICS) {
  ...
  const bump = rows.free15.pvl > 0.3 && rows.free15.meanHz > 2 && Math.abs(rows.free15.angleDeg) < 45;
