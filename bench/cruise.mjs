...
const GL = arg('gl', 'combined'), READOUT = arg('readout', 'dng02'), BACKEND = arg('backend', 'gpu'), SECONDS = Number(arg('seconds', 30)), DNBIAS = Number(arg('dnbias', 0.4)), TURNGAIN = Number(arg('turngain', 2)), HGAIN = Number(arg('halteregain', 2)), BASEAMP = Number(arg('baseamp', 0.7)), RECORD = process.argv.includes('--record');
...
try {
  ...
  for (const haltere of [false, true]) {
    const boot = await openLoop(page, vite.url, { readout: READOUT, backend: BACKEND, dnBias: DNBIAS, turnGain: TURNGAIN, haltere, haltereGain: HGAIN, course: true });
    console.log(`\n== cruise ${SECONDS} s, readout ${READOUT}${READOUT === 'dna02' ? ' (DEVIATION)' : ''}, haltere proxy ${haltere ? 'on (gain ' + HGAIN + ')' : 'off'}; ${boot.backend}, ${boot.config?.pairs} bridge cells`);
  ...
  report.verdict = { pass: report.runs.some((r) => r.collisions === 0 && Math.abs(r.driftDeg) < 20), best: { haltere: best.haltere, collisions: best.collisions, driftDeg: best.driftDeg } };
  console.log(`\nVERDICT ${report.verdict.pass ? 'PASS' : 'FAIL'}: best run haltere ${best.haltere}: ${best.collisions} collisions, drift ${best.driftDeg}° (targets 0 and < 20°)`);
} finally {
